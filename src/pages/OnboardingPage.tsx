...
import {
  ...
  Target, Repeat, CheckSquare, ArrowRight,
} from 'lucide-react';
      ...
      <div className="min-h-screen flex items-center justify-center bg-background p-4">
        <div className="w-full max-w-lg space-y-6">
          <div className="text-center space-y-1">
            <h1 className="text-2xl font-bold tracking-tight">Life<span className="text-primary">OS</span></h1>
            <p className="text-xs text-muted-foreground">Guided setup — Step {guidedStep} of 3</p>
            <Progress value={(guidedStep / 3) * 100} className="h-1.5" />
          </div>
            ...
            <CardContent className="p-6">
              {guidedStep === 1 && (
                <div className="space-y-5">
                  ...
                  <p className="text-[11px] text-muted-foreground text-center">You can skip this — just leave it blank.</p>
              ...
              {guidedStep === 3 && (
                <div className="space-y-5">
                  ...
                  <div className="rounded-lg border border-border bg-muted/30 p-3 text-sm space-y-1">
                    ...
                    {sampleGoal.trim() && <p>🎯 Goal: {sampleGoal.trim()}</p>}
                    {sampleHabit.trim() && <p>🔁 Habit: {sampleHabit.trim()} ({sampleHabitFreq})</p>}
                    {sampleTask.trim() && <p>✅ Task: {sampleTask.trim()}</p>}
                    {!sampleGoal.trim() && !sampleHabit.trim() && !sampleTask.trim() && (
                      <p className="text-muted-foreground">Nothing added yet — that's okay!</p>
                    )}
                  </div>
                  <div className="flex gap-2">
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-background p-4">
      <div className="w-full max-w-lg space-y-6">
        ...
        <Card className={step === 0 ? 'border-0 shadow-none bg-transparent' : ''}>
          <CardContent className={step === 0 ? 'p-0' : 'p-6'}>
            ...
            {step === 1 && (
              <div className="space-y-5">
                <div className="text-center space-y-1">
                  ...
                  <p className="text-sm text-muted-foreground">We'll use this to personalize your dashboard and planning.</p>
                </div>
                <div className="space-y-3">
                  <div>
                    <Label>First name</Label>
                    <Input value={firstName} onChange={e => setFirstName(e.target.value)} placeholder="Your name" autoFocus maxLength={32} />
            ...
            {step === 2 && (
              <div className="space-y-5">
                <div className="text-center space-y-1">
                  ...
                  <p className="text-sm text-muted-foreground">Pick up to 3. This only sets defaults — you can change it anytime.</p>
                </div>
                <div className="grid grid-cols-2 gap-3">
                  {FOCUS_AREAS.map(fa => (
                    <button key={fa.id} onClick={() => toggleFocus(fa.id)}
                      className={`flex flex-col gap-1 rounded-lg border p-3 text-left transition-colors ${focusAreas.includes(fa.id) ? 'border-primary bg-primary/10' : 'border-border hover:bg-muted/50'}`}>
                ...
                <button onClick={() => setNotSureOpen(true)} className="text-xs text-primary hover:underline w-full text-center">Not sure?</button>
                <p className="text-[11px] text-muted-foreground text-center">You can change this later.</p>
                <div className="flex gap-2">
            ...
            {step === 3 && (
              <div className="space-y-5">
                ...
                <div className="space-y-2">
                  {STYLES.map(s => (
                    <button key={s.id} onClick={() => setOperatingStyle(s.id)}
                      ...
                      <div className={`h-3 w-3 rounded-full border-2 shrink-0 ${operatingStyle === s.id ? 'border-primary bg-primary' : 'border-muted-foreground'}`} />
                      <div>
                ...
                <Collapsible open={advancedOpen} onOpenChange={setAdvancedOpen}>
                  ...
                  <CollapsibleContent className="mt-2 space-y-1 text-xs text-muted-foreground rounded-lg border border-border p-3">
                    {STYLES.map(s => (
            ...
            {step === 4 && (
              <div className="space-y-5">
                ...
                <div className="space-y-2">
                  {[
                    { id: 'clean' as const, label: 'Start clean', desc: 'Blank slate. Add what matters when you\'re ready.', icon: Rocket },
                    { id: 'guided' as const, label: 'Guided minimal setup', desc: 'We\'ll help you add 1 goal, 1 habit, and 1 task.', icon: Sparkles },
                    { id: 'demo' as const, label: 'Explore demo', desc: 'Pre-filled example data to click around.', icon: BarChart3 },
                  ].map(opt => (
                    <button key={opt.id} onClick={() => setStartOption(opt.id)}
                      ...
                      {opt.id === 'clean' && <Badge variant="secondary" className="text-[8px] shrink-0">Recommended</Badge>}
                    </button>
                  ))}
                </div>
                <Collapsible>
            ...
            {step === 5 && (
              <div className="space-y-5">
                <div className="text-center space-y-2">
                  ...
                  <p className="text-sm text-muted-foreground">Your workspace is configured and waiting.</p>
                </div>
        ...
        {step >= 2 && step <= 4 && (
          <button onClick={() => setStep(5)} className="text-xs text-muted-foreground hover:text-foreground transition-colors w-full text-center">
            Skip for now
          </button>
        )}
      </div>
